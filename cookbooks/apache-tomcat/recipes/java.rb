# Delete tar.gz file from previous activity
file "/usr/local/#{node[:apache_tomcat][:java_src]}" do
  action :delete
end
...
ruby_block "Install Java" do
        block do
                ...
                system "wget --no-check-certificate --no-cookies --header 'Cookie: oraclelicense=accept-securebackup-cookie'  #{node[:apache_tomcat][:java_url]}"
                system "tar -xzf #{node[:apache_tomcat][:java_src]}"
        end
        action :create
end
...
execute "Make Java link" do
        cwd "/usr/local"
	command "ln -sf #{node[:apache_tomcat][:java_unfold]} java"
end
