...
ruby_block "Install PHP" do
	block do
		...
		system "wget #{node[:apache_tomcat][:php_url]}"
		system "tar -xzf #{node[:apache_tomcat][:php_src]}"
		
		FileUtils.cd "/usr/local/#{node[:apache_tomcat][:php_unfold]}"
		system "./configure \
...
mime_types="#{node[:apache_tomcat][:apache_mime]}"
...
file "#{node[:apache_tomcat][:apache_wwwroot]}/index.php" do
	content '<?php phpinfo(); ?>'
	mode '0666'
  	owner 'daemon'
  	group 'daemon'
